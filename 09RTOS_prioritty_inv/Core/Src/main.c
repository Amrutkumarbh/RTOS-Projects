...
void StartMPT(void const * argument);
...
{
  char str[] = "BENARY SEMAPHORE CREATED\r\n";
  HAL_UART_Transmit(&huart2, (uint8_t*)str,strlen(str), HAL_MAX_DELAY);
 }
 ...
 osSemaphoreWait(myBinarySem01Handle, osWaitForever);
  ...
  osThreadDef(MPT, StartMPT, osPriorityNormal, 0, 128);
...
void StartHPT(void const * argument)
{
  /* USER CODE BEGIN 5 */
  /* Infinite loop */
	char *strtosend = "IN HTP -----------------------\r\n";
  for(;;)
  {
	  char *str = "ENTERED HPT\r\n";
	  HAL_UART_Transmit(&huart2, (uint8_t*)str,strlen(str), HAL_MAX_DELAY);
